...
export default function AdminOAuthApps() {
  ...
  const loadApps = async () => {
    try {
      ...
      setError(err.message || 'Failed to load OAuth apps');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingApp) {
        ...
        setSuccess('OAuth app updated successfully!');
      } else {
        // Create new app
      ...
      setError(err.message || 'Failed to save OAuth app');
    }
  };
  ...
  const handleToggleEnabled = async (app) => {
    try {
      ...
      setSuccess(`OAuth app ${app.is_enabled ? 'disabled' : 'enabled'} successfully!`);
      loadApps();
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="admin-oauth-apps">
        ...
        <div className="apps-list-section">
          ...
          {apps.length === 0 ? (
            <div className="empty-state">
              ...
              <p className="hint">
                Add an OAuth app to allow users to connect their Git platform accounts without editing .env files.
              </p>
            </div>
          ) : (
